...
package org.grameen.mobileSurvey.tasks;
...
import org.grameen.mobileSurvey.R;
import org.grameen.mobileSurvey.activities.MainMenuActivity;
import org.grameen.mobileSurvey.application.Collect;
import org.grameen.mobileSurvey.listeners.InstanceUploaderListener;
import org.grameen.mobileSurvey.preferences.PreferencesActivity;
import org.grameen.mobileSurvey.provider.InstanceProviderAPI;
import org.grameen.mobileSurvey.provider.InstanceProviderAPI.InstanceColumns;
import org.grameen.mobileSurvey.utilities.GFLoginProvider;
import org.grameen.mobileSurvey.utilities.WebUtils;
...
public class InstanceUploaderTask extends AsyncTask<Long, Integer, HashMap<String, String>> {
    ...
    private String mUrl=""; //used in Grameen foundation protocol to storage
    ...
    protected HashMap<String, String> doInBackground(Long... values) {
	        ...
	        if (c.getCount() > 0) {
	            ...
	            while (c.moveToNext()) {
	                ...
	                if (urlString == null) {
	                    ...
	                    mUrl = urlString + submissionUrl;
	                }
	                // ADDED FOR GRAMEEN PROJECT: IF I AM SUBMITTING TO SALESFORCE I HAVE TO FORK FROM NORMAL FLOW
	                // BECAUSE SALESFORCE DOES NOT SUPPORT MULTI-PART CONTENT IN HTTP
	                if (PreferenceManager.getDefaultSharedPreferences(Collect.getInstance()).getString(PreferencesActivity.KEY_PROTOCOL, null).equals(PreferencesActivity.PROTOCOL_GRAMEEN_SALESFORCE)){
				       	mResults = sendDataWithGFSalesforceProtocol(instance,toUpdate);

			        } else if ( !uploadOneSubmission(urlString, id, instance, toUpdate, httpclient, localContext, uriRemap) ) {
        ...
        return mResults;
         ...
         private HashMap<String, String> sendDataWithGFSalesforceProtocol(String instanceFilePath, Uri toUpdate) {
	 		final String TAG = "SALESFORCE_PROTOCOL";
	        ...
	        // figure out what to do with them.
	 	    ...
	 	    File[] allFiles = getFileToSubmit(instanceFilePath, toUpdate,
					instanceFile, cv);
	 	    ...
	 	    for (int i = 0; i < allFiles.length ; i++){
	 	    	...
	 	    	//odk sometimes save .bak files that are useless for us.
	 	    	if(file.getName().endsWith(".bak")){
	 	    		file.delete();
	 	    	}
	 	    	else{
	 		    	try {
	 		    		//we check if it is the last question
	 		    		//if so, we send a parameter in the url
	 		    		//to mark the survey as submitted

	 		    		if (i + 1 == allFiles.length){
	 		    			mUrl+= "?last=true";
	 		    		}
	 		    		HttpPost httpPost = new HttpPost(mUrl);
	 		    		GFLoginProvider.addSalesforceLogin(httpPost);   //we add authentication to header

	 		    		RandomAccessFile rndFile = new RandomAccessFile(file, "r");
	 				    byte[] bytes = new byte[(int) file.length()];
	 				    rndFile.readFully(bytes);

	 				    //we replace the \n to &#13 which is the carriage return in XML
	 				    String content = new String(bytes).replace("\n", "&#13;");
	 				    Log.d(TAG, content);

	 			        // Add your data
	 			        List<NameValuePair> nameValuePairs = new ArrayList<NameValuePair>();
	 			        httpPost.setHeader("Content-Type", "text/xml"); // We are not currently uploading other resource types.
																		// In the future there will be videos, images, etc.

	 			        httpPost.setHeader(new BasicHeader("surveyAnswer", content));    //adding the content of xml
	 			        httpPost.setHeader("surveyAnswer", content);					 //to header
	 			        if(surveyId != null){
	 			        	httpPost.setHeader("surveyId",surveyId);
	 			        }
	 			        ...
	 			        if(surveyId == null){
	 				        ...
	 				        String[] array = response.split("<ID>");
	 				        // I am parsing the response <ID></ID> is expected.
	 				        if(array[1].length()>1){
	 				        	String[] parsedId = array[1].split("</ID>");
	 				        	if(parsedId.length > 0){
	 				        		surveyId = array[1].split("</ID>")[0];
	 				        		isSubmissionOk = true;
	 				        	}
	 				        	//there are some errors on the submission
	 				        	else{
	 				        		cv.put(InstanceColumns.STATUS, InstanceProviderAPI.STATUS_SUBMISSION_FAILED);
	 				        		Collect.getInstance().getContentResolver().update(toUpdate, cv, null, null);
	 				        	}
	 			        ...
	 			        //if the submission was ok , removing from tableview
	 			        if(httpResponse.getStatusLine().getStatusCode() == 200 && isSubmissionOk){
	 			        	toReturn.put(allFiles[i].getName(), allFiles[i].getName());
	 			        	allFiles[i].delete();
	 			        	cv.put(InstanceColumns.STATUS, InstanceProviderAPI.STATUS_SUBMITTED);
	 			        	Collect.getInstance().getContentResolver().update(toUpdate, cv, null, null);
		...
		private File[] getFileToSubmit(String instanceFilePath, Uri toUpdate,
				File instanceFile, ContentValues cv) {
			File submissionFile = new File(instanceFile.getParentFile(), "submission.xml");
	        if ( submissionFile.exists() ) {
	            Log.w(t, "submission.xml will be uploaded instead of " + instanceFile.getAbsolutePath());
	        } else {
	        ...
	        if (!instanceFile.exists() && !submissionFile.exists()) {
	            mResults.put(instanceFilePath, fail + "instance XML file does not exist!");
	            cv.put(InstanceColumns.STATUS, InstanceProviderAPI.STATUS_SUBMISSION_FAILED);
	            Collect.getInstance().getContentResolver().update(toUpdate, cv, null, null);
	        }
